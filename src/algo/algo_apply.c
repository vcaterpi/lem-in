...
/*   Updated: 2020/09/16 16:55:32 by antondob         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		refresh_caps(t_lemin *params)
{
	int curr;
	int prev;
...
static void		find_path(t_lemin *params,
					void *data, t_queue **q)
{
	int i;
...
static void		edmonds_karp(t_lemin *params)
{
	t_queue 	*q;
	int 		i;
	void 		*data;
...
void		apply_algo(t_lemin *params)
{
	if (ERROR)
		return ;
	adapt_capmatrix(params);
	if (!(FLOW_MATRIX = ft_do_table(ROOMS_NUM, ROOMS_NUM)))
		ERROR = 1;
	if (!(PARENT = ft_memalloc(sizeof(int) * ROOMS_NUM)))
		ERROR = 1;
	parent_zero(params);
	edmonds_karp(params);
	adapt_flowmatrix(params); 
}
